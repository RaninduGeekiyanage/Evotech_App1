...
import { dailyMoviesCount } from "@/lib/actions/dashboard/daily-movie";
import { movieCount } from "@/lib/actions/dashboard/movie-count";

const DailyMovieCount = async () => {
  let mCount = 0;
  let error = null;

  try {
    const response = await dailyMoviesCount(); // Fetch data directly on the server
    if (response.success) {
...
export default DailyMovieCount;
